...
from scripts.backtest import detect_opportunities, simulate_opportunity  # noqa: E402
from services.simulator.portfolio import Portfolio  # noqa: E402
...
class TestDetectSkipsResolvedMarkets:
    ...
    async def test_skips_pair_when_market_a_resolved(self):
        """Market A resolved on Jan 5 — detection on Jan 10 should skip it."""
    ...
    async def test_skips_pair_when_market_b_resolved(self):
        """Market B resolved on Jan 5 — detection on Jan 10 should skip it."""
        resolved_date = datetime(2025, 1, 5, tzinfo=timezone.utc)
        as_of = datetime(2025, 1, 10, tzinfo=timezone.utc)
    ...
    async def test_skips_pair_when_both_resolved(self):
        """Both markets resolved — should still skip."""
        resolved_date = datetime(2025, 1, 5, tzinfo=timezone.utc)
        as_of = datetime(2025, 1, 10, tzinfo=timezone.utc)
    ...
    async def test_allows_pair_before_resolution(self):
        ...
        # get_prices_at uses session.execute — return no snapshot so detection exits
        # gracefully (no prices). The point: it was NOT blocked by the resolution check.
        ...
        opp_ids = await detect_opportunities(session, [pair], as_of)
        assert opp_ids == []
        # Verify session.execute was called (meaning we got past the resolution check)
    ...
    async def test_allows_unresolved_pair(self):
        """Neither market resolved — should proceed normally (fail on missing prices)."""
        as_of = datetime(2025, 1, 10, tzinfo=timezone.utc)

        market_a = _make_market(market_id=1)  # resolved_at=None
        market_b = _make_market(market_id=2)  # resolved_at=None
        pair = _make_pair(market_a_id=1, market_b_id=2)
        ...
        opp_ids = await detect_opportunities(session, [pair], as_of)
        assert opp_ids == []
        # Key: session.execute was called = we got past the resolution check
...
class TestSimulateBlocksResolvedMarkets:
    ...
    async def test_blocks_trade_when_market_a_resolved(self):
        """Safety net: even if detection missed it, simulation must block."""
        resolved_date = datetime(2025, 1, 5, tzinfo=timezone.utc)
        as_of = datetime(2025, 1, 10, tzinfo=timezone.utc)
        ...
        def _get_side_effect(model, entity_id):
            call_count[0] += 1
            # Call order: opportunity, pair, market_a, market_b
            if call_count[0] == 1:
                return opp
            elif call_count[0] == 2:
                return pair
        ...
        assert result["status"] == "resolved_market_blocked"
        assert result["trades_executed"] == 0
        assert portfolio.total_trades == 0, "No trades should have been executed"

    @pytest.mark.asyncio
    async def test_blocks_trade_when_market_b_resolved(self):
        """Safety net for market B resolution."""
        resolved_date = datetime(2025, 1, 5, tzinfo=timezone.utc)
        as_of = datetime(2025, 1, 10, tzinfo=timezone.utc)
...
class TestEndToEndResolvedMarketRegression:
    """
    Regression scenario: Market resolves on Day 5. The backtest runs Days 1-10.
    Days 6-10 must NOT produce any new trades on this market.
    """

    @pytest.mark.asyncio
    async def test_no_trades_after_resolution(self):
        """Simulate the exact bug scenario: detect + simulate should both block."""
        resolved_date = datetime(2025, 1, 5, tzinfo=timezone.utc)
        ...
        # Simulate 5 days AFTER resolution (the bug would create trades on each)
