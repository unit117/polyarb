...
ssh $NAS_USER@$NAS_HOST "cd /volume1/docker/polyarb && docker compose build backtest"
```
...
ssh $NAS_USER@$NAS_HOST "cd /volume1/docker/polyarb && \
...
```bash
ssh $NAS_USER@$NAS_HOST "cat > /tmp/run_eval.sh << 'SCRIPT'
#!/bin/bash
set -euo pipefail
...
scp -r $NAS_USER@$NAS_HOST:/volume1/docker/polyarb/eval_results/YYYYMMDD_HHMMSS ./eval_results/
```
